    ...
    .then(data => {
      ...
      feat.innerHTML = `
        <div class="mag-feature__main">
          <div class="mag-feature__hero">${(hero.category||'').slice(0,1).toUpperCase()}</div>
          <div class="mag-feature__cat">${hero.category}</div>
      ...
      grid.innerHTML = grid_items.map(p => `
        <article class="mag-card"><a href="${p.filename}">
          <div class="mag-card__hero">${(p.category||'').slice(0,2).toUpperCase()}</div>
          <div class="mag-card__cat">${p.category}</div>
