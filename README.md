...
### 댓글 (giscus, enabled 기본 ON)

각 포스트가 GitHub Discussion 1개에 매핑. 학생이 GitHub 계정으로 댓글 달면 곧바로 Discussion 에 글이 쌓임. **Disqus 광고·자체 DB 불필요**.

`features.comments.enabled` 는 기본 `true` 지만 giscus.app 의 4 개 data-* 값이 없으면 iframe 자체를 emit 하지 않는다 — 빌드 로그에 안내문 1 회 출력 후 조용히 보류. 4 개 값을 등록한 시점부터 모든 포스트 하단에 자동 노출.
...
### 2026-04-26 — diary 검색·댓글 디폴트 OFF → ON

- `features.search` · `features.comments.enabled` 디폴트가 false → **true** 로 뒤집힘
- 신규 사용자: 별다른 설정 없이 publish 하면 곧바로 Pagefind 검색이 뜨는 사이트가 만들어짐 (Node.js 18+ 필요)
- 기존 사용자 (config 에 `features` 누락): 다음 publish 부터 자동 검색-on. 즉시 적용은 `--rebuild all`
- 댓글: `enabled: true` 디폴트지만 giscus.app 4 개 data-* 값이 없으면 iframe emit 보류 + 빌드 로그에 1 회 안내. 4 개 값 등록한 시점부터 자동 노출
- 끄려면: `/hams:diary config search off` 또는 `/hams:diary config comments off`
